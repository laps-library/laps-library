...
export default function AdminAccountsScreen() {
  ...
  async function load() {
    const { data: acc } = await supabase
      .from('profiles').select('*, plans(name)').order('created_at', { ascending: false });
    setAccounts(acc ?? []);
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {accounts.map((a) => (
          <View key={a.id} style={styles.card}>
            <Text style={styles.name}>{a.first_name || '—'} {a.last_name || ''}</Text>
            <Text style={styles.info}>{a.email}</Text>
            <Text style={styles.info}>Formule : {a.plans?.name ?? '—'}</Text>
            <Text style={styles.info}>Validité : {a.subscription_expires_at ? new Date(a.subscription_expires_at).toLocaleDateString('fr-FR') : '—'}</Text>
            <Text style={styles.info}>Pièce : {a.id_document_status === 'verified' ? 'Vérifiée' : a.id_document_status === 'pending' ? 'En attente' : a.id_document_status === 'rejected' ? 'Refusée' : 'Non envoyée'}</Text>
            <Text style={styles.info}>Rôle : {a.role}</Text>
            <View style={styles.row}>
              {a.role === 'client' ? (
                <Button title="Promouvoir admin" color="#ff2bd6" onPress={() => setRole(a.id, 'admin')} />
              ) : (
                <Button title="Rétrograder client" color="#f87171" onPress={() => setRole(a.id, 'client')} />
              )}
            </View>
            <View style={styles.row}>
              {plans.filter((p) => p.id !== a.plan_id).map((p) => (
                <Button key={p.id} title={p.name} color="#60a5fa" onPress={() => setPlan(a.id, p.id)} />
              ))}
            </View>
          </View>
        ))}
