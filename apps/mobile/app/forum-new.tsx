...
export default function ForumNewScreen() {
  ...
  async function publish() {
    ...
    const { data: thread, error: e1 } = await supabase.from('forum_threads').insert({ title: title.trim() }).select().single();
    if (e1) { Alert.alert('Erreur', e1.message); return; }
    const { error: e2 } = await supabase.from('forum_posts').insert({ thread_id: thread.id, body: body.trim() });
    if (e2) { Alert.alert('Erreur', e2.message); return; }
    router.back();
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder="Titre du sujet" placeholderTextColor="#8e8e93" />

        <Text style={styles.label}>_Message</Text>
        <TextInput style={[styles.input, styles.body]} value={body} onChangeText={setBody} placeholder="Ton message..." placeholderTextColor="#8e8e93" multiline numberOfLines={6} textAlignVertical="top" />
