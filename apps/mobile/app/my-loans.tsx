...
export default function MyLoansScreen() {
  ...
  const statusLabel = (s: string, startDate?: string) => {
    ...
    switch (s) {
      case 'requested': return 'En attente de validation';
          ...
          return (
            <View style={styles.card}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <View style={{ backgroundColor: statusColor(item.status), paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 }}>
              ...
              {item.start_date && (
                <Text style={styles.meta}>
                  Début : {new Date(item.start_date).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric' })}
                </Text>
              )}

              {item.due_at && (
                <Text style={styles.meta}>
                  Retour prévu : {new Date(item.due_at).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric' })}
                </Text>
              )}
              ...
              {item.payment_status && (
                <Text style={styles.meta}>
                  Paiement : {item.payment_status === 'paid' ? '✅ Payé' : item.payment_status === 'unpaid' ? '⏳ En attente' : item.payment_status}
                </Text>
              )}
              ...
              {canModify && (
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                  <TouchableOpacity
                    onPress={() => router.push({ pathname: '/modify/[id]', params: { id: item.id, type: 'loan' } })}
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 17, fontWeight: 'bold', fontStyle: 'italic', textTransform: 'uppercase', color: '#fff', letterSpacing: 1, flex: 1 },
  meta: { color: '#ccc', fontSize: 13 },
  btnModify: { backgroundColor: '#ff2bd6', borderRadius: 8, paddingVertical: 8, paddingHorizontal: 14 },
  btnCancel: { backgroundColor: '#f87171', borderRadius: 8, paddingVertical: 8, paddingHorizontal: 14 },
  btnText: { color: '#fff', fontWeight: '700', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 },
});
