...
type Res = {
  ...
  price_cents: number;
  status: string;
  workstations: { name: string };
  time_slots: { name: string };
  instrument_models?: { name: string };
};
...
export default function MyReservations() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from("reservations")
        .select(
          "id, reservation_date, start_time, price_cents, status, workstations(name), time_slots(name), instrument_models(name)",
        )
        .eq("user_id", uid)
  ...
  function canModify(r: Res) {
    ...
    return (
      start.getTime() - Date.now() > H16 &&
      (r.status === "confirmed" || r.status === "pending_validation")
    );
  }
        ...
        {list.map((r) => (
          <View key={r.id} style={styles.card}>
            <Text style={styles.name}>
              {(r as any).instrument_models?.name
                ? (r as any).instrument_models.name.replace("Poste Premium — ", "")
                : r.workstations?.name}
            </Text>
