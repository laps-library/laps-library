...
export default function WaitingPaymentScreen() {
  ...
  async function checkPayment() {
    ...
    const { data: p } = await supabase.from('profiles').select('plan_id, subscription_expires_at').eq('id', uid).single();
    ...
    if (free) { router.replace('/home'); return; }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <AppButton label="Retour au choix" fontSize={10} onPress={() => router.replace('/choose-plan')} />
...
const styles = StyleSheet.create({
  ...
  title: { fontWeight: 'bold', fontStyle: 'italic', textTransform: 'uppercase', fontSize: 22, color: '#fff', textAlign: 'center', letterSpacing: 1 },
